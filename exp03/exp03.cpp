...
int main()
{
    ...
    int a;
    test.get(AGE, a);
    std::cout << "(测试) 田所浩二的年龄: " << a << std::endl;
    char g;
    test.get(GENDER, g);
    std::cout << "(测试) 田所浩二的性别: " << g << std::endl;
    const char* cn;
    test.get(CLASS_NAME, cn);
    std::cout << "(测试) 田所浩二的班级: " << cn << std::endl;
    double gc;
    test.get(GRADE_COMP, gc);
    std::cout << "(测试) 田所浩二的计算机成绩: " << gc << std::endl;
    int ge;
    test.get(GRADE_ENG, ge);
    std::cout << "(测试 | int) 田所浩二的英语成绩: " << ge << std::endl;
    std::cout << "(测试) 田所浩二的平均成绩: " << test.avg() << std::endl;
    // ================ 执行部分 ================
